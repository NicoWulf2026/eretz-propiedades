...
export function Pagination({ filters, hasNext, hasPrevious, nextCursor, previousCursor, basePath = "/" }: PaginationProps) {
  ...
  return (
    <nav aria-label="Paginación" className="mt-10 flex items-center justify-between gap-4">
      {hasPrevious ? <Link className="secondary-button" href={href(Math.max(1, filters.page - 1), previousCursor ?? "", "prev")}>← Anterior</Link> : <span />}
      <p className="text-sm font-semibold u-text-muted">Página {filters.page.toLocaleString("es-AR")}</p>
      {hasNext ? <Link className="secondary-button" href={href(filters.page + 1, nextCursor ?? "", "next")}>Siguiente →</Link> : <span />}
    </nav>
  );
}
