      ...
      {open ? (
        <div className="collection-picker-panel" role="group" aria-label="Colecciones">
          {collections.length === 0 ? (
            <p className="text-sm text-slate-600">Todavía no tenés colecciones. Creá una:</p>
          ) : (
                ...
                return (
                  <li key={c.id}>
                    <label className="flex items-center gap-2 text-sm text-slate-700">
                      ...
                      />
                      {c.name} <span className="text-slate-400">({c.ids.length})</span>
                    </label>
                  </li>
                );
              })}
          ...
          >
            ...
            <input id={`newcol-${propertyId}`} value={newName} onChange={(e) => setNewName(e.target.value)} maxLength={60} placeholder="Nueva colección" className="flex-1 rounded-lg border border-slate-300 px-2 py-1 text-sm" />
