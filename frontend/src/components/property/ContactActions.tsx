...
export function ContactActions({ property, canonical }: { property: Property; canonical: string }) {
  ...
  return (
    <aside className="detail-panel rounded-2xl border p-5">
      <p className="text-xs font-bold uppercase tracking-[0.16em] text-[color:var(--accent-soft)]">Contacto del aviso</p>
      <h2 className="mt-2 text-xl font-black text-[color:var(--ink)]">{contact?.name ?? property.agentName ?? "Responsable de la publicación"}</h2>
      {contact?.verified ? <p className="mt-1 text-xs font-bold u-ok-text">Identidad verificada en ERETZ</p> : null}
      <p className="mt-2 text-sm leading-6 u-text-muted">ERETZ no recibe ni almacena tu consulta. Usá un canal público disponible o continuá en la publicación original.</p>
      {(whatsapp || contact?.email) ? (
        <fieldset className="mt-4 rounded-xl border u-border p-3">
          <legend className="px-1 text-xs font-bold u-text-muted">¿Sobre qué querés consultar? (opcional)</legend>
          <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1.5">
            {CONTACT_TOPICS.map((t) => (
              <label key={t.id} className="check-label inline-flex items-center gap-1.5 text-sm u-text-muted">
      ...
      <div className="detail-contact-actions">
        ...
        {!whatsappHref && !telephone && !contact?.email && !contact?.website && property.sourceUrl ? <a className="primary-button justify-center" href={property.sourceUrl} target="_blank" rel="noopener noreferrer nofollow" onClick={() => trackContact("source_publication")}>Ver publicación original ↗</a> : null}
      </div>
      ...
      <div className="mt-5 flex flex-wrap gap-3 border-t u-border pt-4">
        <a className="inline-action text-sm font-bold text-[color:var(--accent-soft)]" href={shareHref} target="_blank" rel="noopener noreferrer" onClick={() => track("share_clicked", { method: "whatsapp" })}>Compartir por WhatsApp</a>
        <a className="inline-action text-sm font-bold text-[color:var(--accent-soft)]" href={`mailto:?subject=${encodeURIComponent(property.title)}&body=${encodeURIComponent(shareMessage)}`} onClick={() => track("share_clicked", { method: "email" })}>Compartir por correo</a>
        <button className="inline-action text-sm font-bold text-[color:var(--accent-soft)]" type="button" onClick={webShare}>Compartir</button>
        <button className="inline-action text-sm font-bold text-[color:var(--accent-soft)]" type="button" onClick={copy}>{copied ? "Enlace copiado" : "Copiar enlace"}</button>
      </div>
    </aside>
  );
}
