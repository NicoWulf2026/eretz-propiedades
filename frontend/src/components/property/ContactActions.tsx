...
export function ContactActions({ property, canonical }: { property: Property; canonical: string }) {
  ...
  return (
    <aside className="detail-panel rounded-2xl border p-5">
      <p className="text-xs font-bold uppercase tracking-[0.16em] text-[color:var(--accent-soft)]">Contacto directo</p>
      ...
      <p className="mt-2 text-sm leading-6 u-text-muted">ERETZ no recibe ni almacena tu consulta. Te conectamos con la publicación original.</p>
      ...
      <div className="mt-5 flex flex-wrap gap-3 border-t u-border pt-4">
        <a className="inline-action text-sm font-bold text-[color:var(--accent-soft)]" href={shareHref} target="_blank" rel="noopener noreferrer" onClick={() => track("share_clicked", { method: "whatsapp" })}>WhatsApp</a>
        <a className="inline-action text-sm font-bold text-[color:var(--accent-soft)]" href={`mailto:?subject=${encodeURIComponent(property.title)}&body=${encodeURIComponent(shareMessage)}`} onClick={() => track("share_clicked", { method: "email" })}>Correo</a>
