    ...
    <header className="sticky top-0 z-[1500] border-b border-white/10 bg-[#06192f]/97 text-white backdrop-blur">
      <nav className="container flex h-16 items-center justify-between" aria-label="Navegación principal">
        ...
        <div className="hidden items-center gap-1 lg:flex">
          ...
          <Link href="/terminos" className="rounded-lg px-3 py-2 text-sm font-semibold text-white/75 hover:bg-white/10 hover:text-white">
            Términos
          </Link>
          <Link href="/privacidad" className="nav-contact ml-1 rounded-lg bg-white px-4 py-2.5 text-sm font-bold hover:bg-[#f4e8cc]">
            Privacidad
          </Link>
        </div>
        <button
      ...
      {open && (
        <div id="mobile-navigation" className="border-t border-white/10 bg-[#06192f] lg:hidden">
          <div className="container flex flex-col py-3">
            ...
            <Link href="/terminos" onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 font-semibold text-white/85">Términos</Link>
            <Link href="/privacidad" onClick={() => setOpen(false)} className="nav-contact mt-2 rounded-lg bg-white px-3 py-3 text-center font-bold">Privacidad</Link>
          </div>
        </div>
      )}
    </header>
