  ...
  return (
    <SiteShell>
      ...
      <div className="container py-8">
        <nav aria-label="Migas de pan" className="flex flex-wrap items-center gap-2 text-sm text-slate-600"><Link href={returnTo} scroll={false} className="font-bold text-[color:var(--brand)]">← Volver a resultados</Link><span aria-hidden="true">/</span><span aria-current="page">{typeLabels[property.propertyType]}</span></nav>
        <header id="resumen" className="mt-6 scroll-mt-24">
          ...
          <div className="mt-4 flex flex-wrap items-end justify-between gap-3"><div><PriceTag property={property} className="detail-price" /><p className="mt-2 text-sm text-slate-600">{propertyLocation(property)}</p>{property.publisher ? <p className="mt-2 text-sm font-bold text-[color:var(--ink)]">Publicado por {property.publisher.name}</p> : null}</div><div className="text-right">{updated && <p className="text-xs text-slate-600">Actualizado el {updated}</p>}<p className="mt-1 text-xs text-slate-600">ID ERETZ {property.id}</p></div></div>
        </header>
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.6fr)_minmax(320px,.7fr)]">
          ...
          <a href="#aviso">Información del aviso</a>
            ...
            <section id="publicador" className="detail-panel scroll-mt-24"><h2>Publicado por</h2>{property.publisher ? <p className="mt-3 text-slate-700">{property.publisher.id ? <Link href={`/inmobiliaria/${entitySlug(property.publisher.id, property.publisher.name)}`} className="font-bold text-[color:var(--brand)] underline-offset-2 hover:underline">{property.publisher.name}</Link> : property.publisher.name}{property.publisher.verified ? " · Verificada" : ""}</p> : <p className="mt-3 text-slate-600">Publicador no especificado.</p>}{property.publisher?.id ? <p className="mt-2 text-sm"><Link href={`/inmobiliaria/${entitySlug(property.publisher.id, property.publisher.name)}`} className="inline-action font-bold text-[color:var(--brand)]">Ver todas sus propiedades →</Link></p> : null}<p className="mt-2 text-sm text-slate-500">Los datos de contacto están en el panel lateral.</p></section>
            <section id="aviso" className="detail-panel scroll-mt-24"><h2>Información del aviso</h2><div className="mt-3 grid gap-2 text-sm text-slate-600 sm:grid-cols-2"><p>Operación: {operationLabels[property.operation]}</p><p>Precio: {propertyPrice(property)}</p>{updated ? <p>Actualizado el {updated}</p> : null}<p>ID ERETZ: {property.id}</p></div>{property.sourceUrl ? <p className="mt-3 text-sm"><a className="inline-action font-bold text-[color:var(--brand)]" href={property.sourceUrl} target="_blank" rel="noopener noreferrer nofollow">Ver aviso original</a></p> : null}<p className="mt-3 text-xs leading-5 text-slate-500"><strong>Información de terceros.</strong> El precio, la disponibilidad y las características pueden cambiar. Confirmá con el responsable de la publicación.</p><div className="mt-4"><ReportButton propertyId={property.id} /></div></section>
          </div>
        ...
        {priceHistory.length > 1 && <section className="mt-16"><p className="eyebrow">Transparencia</p><h2 className="section-title">Historial de precio</h2><ul className="mt-4 divide-y divide-slate-100 rounded-xl border border-slate-200">{priceHistory.map((point, i) => <li key={i} className="flex items-center justify-between gap-3 p-3 text-sm"><span className="font-bold text-[color:var(--ink)]">{point.currency ? `${point.currency} ` : ""}{point.price.toLocaleString("es-AR")}</span><time className="text-slate-600" dateTime={point.at}>{formatDate(point.at)}</time></li>)}</ul></section>}
