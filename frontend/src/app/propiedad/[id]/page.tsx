      ...
      <div className="container py-8">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.6fr)_minmax(320px,.7fr)]">
          <div className="min-w-0 space-y-8">
            ...
            <section id="publicador" className="detail-panel scroll-mt-24"><h2>Publicado por</h2>{property.publisher ? <p className="mt-3 text-slate-700">{property.publisher.id ? <Link href={`/inmobiliaria/${entitySlug(property.publisher.id, property.publisher.name)}`} className="font-bold text-[#2166a5] underline-offset-2 hover:underline">{property.publisher.name}</Link> : property.publisher.name}{property.publisher.verified ? " · Verificada" : ""}</p> : <p className="mt-3 text-slate-600">Publicador no especificado.</p>}{property.publisher?.id ? <p className="mt-2 text-sm"><Link href={`/inmobiliaria/${entitySlug(property.publisher.id, property.publisher.name)}`} className="font-bold text-[#2166a5]">Ver todas sus propiedades →</Link></p> : null}<p className="mt-2 text-sm text-slate-500">Los datos de contacto están en el panel lateral.</p></section>
