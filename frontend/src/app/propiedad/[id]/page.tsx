      ...
      <div className="container py-8">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.6fr)_minmax(320px,.7fr)]">
          <div className="min-w-0 space-y-8">
            ...
            <section id="publicador" className="detail-panel scroll-mt-24"><h2>Publicado por</h2>{property.publisher ? <p className="mt-3 text-slate-700">{property.publisher.id ? <Link href={`/inmobiliaria/${entitySlug(property.publisher.id, property.publisher.name)}`} className="font-bold text-[#2166a5] underline-offset-2 hover:underline">{property.publisher.name}</Link> : property.publisher.name}{property.publisher.verified ? " · Verificada" : ""}</p> : <p className="mt-3 text-slate-600">Publicador no especificado.</p>}{property.publisher?.id ? <p className="mt-2 text-sm"><Link href={`/inmobiliaria/${entitySlug(property.publisher.id, property.publisher.name)}`} className="inline-action font-bold text-[#2166a5]">Ver todas sus propiedades →</Link></p> : null}<p className="mt-2 text-sm text-slate-500">Los datos de contacto están en el panel lateral.</p></section>
            <section id="aviso" className="detail-panel scroll-mt-24"><h2>Información del aviso</h2><div className="mt-3 grid gap-2 text-sm text-slate-600 sm:grid-cols-2"><p>Operación: {operationLabels[property.operation]}</p><p>Precio: {propertyPrice(property)}</p>{updated ? <p>Actualizado el {updated}</p> : null}<p>ID ERETZ: {property.id}</p></div>{property.sourceUrl ? <p className="mt-3 text-sm"><a className="inline-action font-bold text-[#2166a5]" href={property.sourceUrl} target="_blank" rel="noopener noreferrer nofollow">Ver aviso original</a></p> : null}<p className="mt-3 text-xs leading-5 text-slate-500"><strong>Información de terceros.</strong> El precio, la disponibilidad y las características pueden cambiar. Confirmá con el responsable de la publicación.</p><div className="mt-4"><ReportButton propertyId={property.id} /></div></section>
          </div>
          <div id="contacto" className="min-w-0 scroll-mt-24 lg:sticky lg:top-24 lg:self-start"><ContactActions property={property} canonical={canonical} /><div className="mt-3"><CollectionPicker propertyId={property.id} /></div><div className="mt-4 rounded-2xl bg-amber-50 p-5 text-xs leading-5 text-amber-950"><strong>Información de terceros.</strong> El precio, la disponibilidad y las características pueden cambiar. Confirmá los datos con el responsable de la publicación. ERETZ no responde por errores del aviso original.</div></div>
        </div>
        {priceHistory.length > 1 && <section className="mt-16"><p className="eyebrow">Transparencia</p><h2 className="section-title">Historial de precio</h2><ul className="mt-4 divide-y divide-slate-100 rounded-xl border border-slate-200">{priceHistory.map((point, i) => <li key={i} className="flex items-center justify-between gap-3 p-3 text-sm"><span className="font-bold text-[#0b2748]">{point.currency ? `${point.currency} ` : ""}{point.price.toLocaleString("es-AR")}</span><time className="text-slate-600" dateTime={point.at}>{formatDate(point.at)}</time></li>)}</ul></section>}
