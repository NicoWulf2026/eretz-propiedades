...
def test_map_first_real_server_flow_and_no_supabase_browser_requests(page: Page) -> None:
    ...
    expect(page.locator(".eretz-map-cluster, .eretz-price-marker").first).to_be_visible(timeout=30_000)
...
def test_search_filter_map_area_detail_and_restoration(page: Page) -> None:
    page.goto(BASE_URL, wait_until="domcontentloaded")
    search = page.get_by_role("combobox", name="Buscar por ubicación, dirección, tipo o inmobiliaria")
    search.fill("Palermo")
    ...
    if first_card.count() == 0:
        pytest.skip("The selected real-data query currently has no visible cards")
    first_card.click()
    page.wait_for_url("**/propiedad/**")
    expect(page.get_by_text("Contacto directo")).to_be_visible()
    expect(page.get_by_text("ID ERETZ", exact=False)).to_be_visible()
...
def test_mobile_map_results_and_missing_property(page: Page) -> None:
    ...
    page.get_by_role("button", name="Resultados", exact=True).click()
    expect(page.locator("[data-property-id]").first).to_be_visible()
