...
def test_se_lee_el_diagnostico_de_cada_agencia(tmp_path):
    ...
    # Cada agencia guarda una LISTA de entradas, aunque tenga una sola.
    assert "JavaScript" in fuera["roomix:armanino"][0]["diagnostico"]
    # Sin firma: difieren del corte por lote y de nada mas.
    assert fuera["roomix:armanino"][0]["componente"] == ""
    assert fuera["roomix:armanino"][0]["radio"] == ""
