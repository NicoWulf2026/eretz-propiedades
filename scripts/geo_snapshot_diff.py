...
def comparar(viejo: dict[str, dict[str, Any]],
    ...
    for identificador in sorted(set(viejo) & set(nuevo)):
        ...
        provincia_antes = (antes.get("provincia") or {}).get("nombre")
        provincia_despues = (despues.get("provincia") or {}).get("nombre")
        if provincia_antes != provincia_despues:
            mudados.append({"id": identificador,
                            "nombre": despues.get("nombre"),
                            "antes": provincia_antes,
                            "despues": provincia_despues})
    return {
...
def main() -> int:
    ...
    for recurso in RECURSOS:
        ...
        riesgo += resultado["eliminados"] + resultado["cambiaron_de_provincia"]
    ...
    print()
    if riesgo:
        print(f"REVISAR ANTES DE REEMPLAZAR: {riesgo} entidades eliminadas o "
