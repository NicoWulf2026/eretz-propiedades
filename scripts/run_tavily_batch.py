...
def main() -> int:
    ...
    pendientes = [json.loads(l) for l in
                  (dd / "web_search_unresolved.jsonl").open(encoding="utf-8") if l.strip()]
    ...
    buscador = sp.ConCache(sp.Tavily(tope=a.tope), dd / "search_cache.jsonl")
    if not buscador.disponible():
        print("TAVILY_API_KEY ausente: no se ejecuta nada.", flush=True)
        return 3

    print("### LOTE TAVILY (nivel gratuito) ###", flush=True)
    ...
    (dd / "web_search_unresolved_after_tavily.jsonl").write_text(
    ...
    print(f"  creditos restantes aprox: {max(0, 1500 - interno.emitidas):,}", flush=True)
    return 0
