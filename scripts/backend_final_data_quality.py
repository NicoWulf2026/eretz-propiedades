...
RULES = [
    ...
    ("url", "INVALID_URL_FORMAT", "INTERNAL_CORRECTABLE", "NORMALIZATION", "high", "url IS NOT NULL AND btrim(url) <> '' AND url !~* '^https?://[^/]+'", "invalid HTTP(S) URL"),
    ...
    ("url", "HOME_OR_NON_DETAIL_URL", "INTERNAL_CORRECTABLE", "VALIDATION", "high", "lower(regexp_replace(coalesce(url,''), '[?#].*$', '')) ~ '^https?://[^/]+/?$|/(contacto|contact|nosotros|blog|noticias|login)/?$|/(page|pagina)/[0-9]+/?$'", "non-detail route"),
    ("url_normalizada", "MISSING_NORMALIZED_URL", "INTERNAL_CORRECTABLE", "NORMALIZATION", "medium", "url IS NOT NULL AND btrim(url) <> '' AND (url_normalizada IS NULL OR btrim(url_normalizada) = '')", "canonical URL key missing"),
    ...
    ("moneda", "INVALID_CURRENCY", "INTERNAL_CORRECTABLE", "NORMALIZATION", "high", "precio IS NOT NULL AND precio > 0 AND lower(btrim(coalesce(moneda,''))) NOT IN ('ars','usd','eur','pesos','dólares','dolares')", "missing or unsupported currency"),
